...
export default class LoggingSession {
  ...
  constructor(filepath, timestampString = Date.now(), session_id, highlightState) {
    ...
    const date = Date(timestampString); // Dateオブジェクトを作成
    // 各要素を取得
    /*const year = date.getFullYear();          // 年 (4桁)
    const month = date.getMonth() + 1;       // 月 (0から始まるので+1)
    const day = date.getDate();              // 日
    const hours = date.getHours();           // 時間
    const minutes = date.getMinutes();       // 分
    const seconds = date.getSeconds();       // 秒
    
    const nomalDate = `${year}Y${month}M${day}D-${hours}h${minutes}m${seconds}s`;*/
    const nomalDate = date.toISOString();
      
    //this._baseFilePathWithFolder = fileOutputBasePath;
    this._gazeFilePath = `${fileOutputBasePath}itrace_atom-${nomalDate}.xml`;
